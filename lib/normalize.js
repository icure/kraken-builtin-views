...
var sieve = function(strings, max_length, max_length_for_split, do_emit, do_else) {
    if (max_length != null && strings.some(it => it.length > max_length)) {
        do_emit(null)
    }
    if (max_length_for_split != null) {
        strings.filter(it => it.length > max_length_for_split && (max_length == null || it.length <= max_length)).forEach(it => {
            do_emit(do_normalize(it.toLowerCase()))
        })
    }
    var remaining = strings.filter(it => max_length_for_split == null || it.length <= max_length_for_split)
...
var do_normalize_and_substring = function(strings, max_length, max_length_for_split, do_emit) {
    sieve(
        strings,
        max_length,
        max_length_for_split,
